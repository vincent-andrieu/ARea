...
export async function UpdateProfileBanner(text: string): Promise<void> {
    if (!env.TWITTER_API_KEY || !env.TWITTER_API_SECRET_KEY)
        return;
    var client = new TwitterApi({
        appKey: env.TWITTER_API_KEY,
    ...
    // client.v1.updateAccountProfileImage("./path/to/file.jpg");
    client.v1.updateAccountProfileBanner("/path/to/image.png");
}